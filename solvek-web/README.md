...
| [Board](components/board/README.md) | 공지사항·알림 게시판 목록 테이블 (유형 + 제목 + 게시일, 신규 게시물 배지) |

> CSS는 `../css/component/{header,footer,filter,pagination,board}.css`에 있으며 `../css/index.css`가 통합 호출합니다 — 위치는 그대로 유지됩니다. (Applied Filter도 `filter.css`에 포함, Card/List는 별도 CSS 없이 기존 유틸리티/`meta-tag` 스타일만 사용)
...
| [`board.html`](board.html) | [Board](components/board/README.md) |
